...
<?php
    /*echo '<div class="medicines_wrap" id="medicinesWrap">
        ...
        $field_medicine = '<div class="medicines_row clearfix">';
            $field_medicine .= '<div class="col-sm-6 medicine_width">';
                $field_medicine .= '<div class="form-row">';
                    $field_medicine .= '<label class="name">Medicines<span class="required" aria-required="true"></span></label>';
                        $field_medicine .= '<div class="inputs">';
                            $field_medicine .=  $this->Form->input('medicines.medicine_id[]', ['options' => $medicines, 'class'=>'form-control', 'default' => (isset($prescription_medicine->medicine_id))? $prescription_medicine->medicine_id:'', 'empty' => 'Select', 'class'=>'form-control selectpicker', 'data-live-search'=>true, 'label'=>false]);
                    $field_medicine .= '</div>';
                $field_medicine .= '</div>';
            $field_medicine .= '</div>';
    ...
    echo '<div class="tests_wrap" id="testsWrap">
        ...
        $field_test = '<div class="medicines_row clearfix">';
            $field_test .= '<div class="col-sm-6 test_width">';
                $field_test .= '<div class="form-row">';
                    $field_test .= '<label class="name">Tests<span class="required" aria-required="true"></span></label>';
                         $field_test .= '<div class="inputs">';
                             $field_test .=  $this->Form->input('tests.test_id[]', ['options' => $tests, 'class'=>'form-control', 'default' => (isset($prescription_test->test_id))? $prescription_test->test_id:'', 'empty' => 'Select', 'class'=>'form-control selectpicker', 'data-live-search'=>true, 'label'=>false]);
                    $field_test .= '</div>';
                $field_test .= '</div>';
            $field_test .= '</div>';
