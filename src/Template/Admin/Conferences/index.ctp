<div class="workspace-dashboard page page-ui-tables">
    <div class="page-heading">
        <div class="flex-container">
            <div class="flex-item"><h4><?= __('Conferences') ?></h4></div>
            <div class="flex-item">
                <div class="flex-container">
                    ...
                    echo $this->Html->link(
                        ...
                        ['action' => 'add'],
                        ['class' => 'add-event-btn', 'escapeTitle' => false, 'title' => 'Add Conference']
                    )
                    ?>
                </div>
            </div>
    ...
    <div class="event-listing">
        <div class="event-listing-top flex-container status-function">
            <div class="status-area flex-container">
                <div class="event-src-box">
                    <?php echo $this->Form->create('Conferences',['type' => 'get'],array('id' => 'site-search','url'=>array('action'=>'index'),'method'=>'get'));?>
        ...
        <div class="table-responsive table-part">
            <table class="table table-hover  table-striped">
                <thead>
                    <tr>
                        ...
                        <th><?= $this->Paginator->sort('conference_date') ?></th>
                ...
                <tbody>
                    ...
                    <?php foreach ($conferences as $conference){ ?>
                        <?php $bulk_delete_fields .= '<input type="hidden" name="ids[]" id="bulk-delete-id-'.$conference->id.'" />' ?>
                    <tr>
                        <td><label><input type="checkbox" value="<?= $conference->id ?>"  class="bulk-delete"> &nbsp; <?= h($conference->name) ?></label></td>
                        <td><?= h($conference->slug) ?></td>
                        <td><?= h($this->Common->getDate($conference->conference_date)) ?></td>
                        <td><?= h($conference->start_time) ?></td>
                        <td><?= h($conference->end_time) ?></td>
                        <td><?= h($conference->venue->name)?></td>
                        <td><?= h($conference->event->name)?></td>
                            ...
                            <div class="dropdown action-button">
                                ...
                                <ul class="dropdown-menu action-dropdown">
                                    <li>
                                        ...
                                        ['action' => 'edit', $conference->id],
                                        ['escapeTitle' => false, 'title' => 'Edit Conference']
                                        );
                                        ?>
                                    </li>
                                    <li>
                                        ...
                                        ['escapeTitle' => false, 'title' => 'Delete Conference','confirm' => __('Are you sure you want to delete # {0}?', $conference->id)]
                                        );
                                        ?>
                                    </li>
                                </ul>
            ...
            <?php
            echo $this->Form->create('Conferences', ['url' => ['action' => 'bulk-delete' ], 'name' => 'form_bulk_delete' ] );
